...
class MCTSRolloutPolicy:
    ...
    def __init__(
        self,
        rollout_duration_seconds: float = 12.0,  # K: how far into the future to look
    ...
    def _hash_state(self, sim: RailwayCrossingSimulator, action: PolicyAction) -> str:
        """
        ...
        return f"{snap.barrier_closed}_{snap.queue_counts['left']}_{snap.queue_counts['right']}_{action.name}"
    ...
    def decide(self, simulator: RailwayCrossingSimulator) -> PolicyAction:
        ...
        for action in simulator.config.actions:
            state_hash = self._hash_state(simulator, action)
